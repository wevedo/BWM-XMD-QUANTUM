...
const groupSessionBuilt = new Map();
const memberSessions = new Map();
const sessionBuildQueue = new Map();
const messageDelay = new Map();

// Session building configuration
const SESSION_CONFIG = {
    BUILD_DELAY: 3000,           // 3 seconds delay before building
    MESSAGE_DELAY: 2000,         // 2 seconds delay after receiving message
    PRESENCE_INTERVAL: 30000,    // 30 seconds presence updates
    RETRY_DELAY: 5000,           // 5 seconds between retries
    MAX_RETRIES: 3,              // Maximum retry attempts
    MEMBER_BUILD_BATCH: 5        // Build sessions for 5 members at a time
};

// Enhanced session error tracking
const sessionErrors = new Map();

function trackSessionError(jid, error) {
    const now = Date.now();
    ...
    errorData.needsRebuild = true;
    
    console.log(`🚨 Session error for ${jid}: ${error.message} (Count: ${errorData.count})`);
    
    // Schedule session rebuild
    scheduleSessionRebuild(jid);
    
    return errorData.count >= 3;
}

// 1. PRESENCE SUBSCRIPTION METHOD
async function subscribeToPresence(jid) {
    try {
        if (adams && adams.presenceSubscribe) {
            await adams.presenceSubscribe(jid);
            console.log(`✅ Subscribed to presence for ${jid}`);
            return true;
        }
    } catch (error) {
        console.error(`❌ Failed to subscribe to presence for ${jid}:`, error);
    }
    return false;
}

// 2. RECEIVE MESSAGE FIRST METHOD (Wait for incoming message)
function waitForIncomingMessage(jid) {
    return new Promise((resolve) => {
        const timeout = setTimeout(() => {
            console.log(`⏰ Timeout waiting for message from ${jid}`);
            resolve(false);
        }, 30000); // 30 second timeout

        const messageListener = (msg) => {
            const message = msg.messages[0];
            if (message?.key?.remoteJid === jid && !message.key.fromMe) {
                clearTimeout(timeout);
                adams.ev.off("messages.upsert", messageListener);
                console.log(`📨 Received message from ${jid}, session should be ready`);
                resolve(true);
            }
        };

        adams.ev.on("messages.upsert", messageListener);
    });
}

// 3. DELAYED REPLY METHOD (Wait before responding)
async function delayedResponse(jid, delay = SESSION_CONFIG.MESSAGE_DELAY) {
    console.log(`⏳ Delaying response for ${jid} by ${delay}ms`);
    await new Promise(resolve => setTimeout(resolve, delay));
    
    // Mark session as ready
    sessionTracking.set(jid, {
        ready: true,
        lastActivity: Date.now(),
        method: 'delayed_response'
    });
}

// 4. BUILD SESSIONS FOR GROUP MEMBERS METHOD
async function buildGroupMemberSessions(groupJid) {
    try {
        if (groupSessionBuilt.has(groupJid)) {
            console.log(`✅ Group sessions already built for ${groupJid}`);
            return true;
        }

        console.log(`🔨 Building sessions for group members: ${groupJid}`);
        
        const groupMetadata = await adams.groupMetadata(groupJid);
        const participants = groupMetadata.participants;
        
        console.log(`👥 Found ${participants.length} members in group`);
        
        // Build sessions in batches to avoid rate limiting
        const batches = [];
        for (let i = 0; i < participants.length; i += SESSION_CONFIG.MEMBER_BUILD_BATCH) {
            batches.push(participants.slice(i, i + SESSION_CONFIG.MEMBER_BUILD_BATCH));
        }
        
        for (const batch of batches) {
            const sessionPromises = batch.map(async (participant) => {
                try {
                    const memberJid = participant.id;
                    
                    // Try multiple session building methods
                    await subscribeToPresence(memberJid);
                    await adams.sendPresenceUpdate("available", memberJid);
                    
                    // Try to fetch profile picture (this helps build sessions)
                    try {
                        await adams.profilePictureUrl(memberJid, 'image');
                    } catch (ppError) {
                        // Profile picture fetch failed, but that's okay
                    }
                    
                    memberSessions.set(memberJid, {
                        groupJid,
                        built: true,
                        timestamp: Date.now()
                    });
                    
                    console.log(`✅ Built session for member: ${memberJid}`);
                    return true;
                } catch (error) {
                    console.error(`❌ Failed to build session for member ${participant.id}:`, error);
                    return false;
                }
            });
        ...
        groupSessionBuilt.set(groupJid, {
            built: true,
            timestamp: Date.now(),
            memberCount: participants.length
        });
        
        console.log(`🎉 Completed building sessions for group: ${groupJid}`);
        return true;
    } catch (error) {
        console.error(`❌ Failed to build group member sessions for ${groupJid}:`, error);
        return false;
    }
}

// Schedule session rebuild
function scheduleSessionRebuild(jid) {
    if (sessionBuildQueue.has(jid)) {
        console.log(`🔄 Session rebuild already scheduled for ${jid}`);
        return;
    }
    
    console.log(`📅 Scheduling session rebuild for ${jid}`);
    
    sessionBuildQueue.set(jid, {
        scheduled: Date.now(),
        attempts: 0
    });
    
    setTimeout(async () => {
        await rebuildSessionForJid(jid);
    }, SESSION_CONFIG.BUILD_DELAY);
}

// Rebuild session for specific JID
async function rebuildSessionForJid(jid) {
    try {
        const queueData = sessionBuildQueue.get(jid);
        if (!queueData || queueData.attempts >= SESSION_CONFIG.MAX_RETRIES) {
            sessionBuildQueue.delete(jid);
            console.log(`🚫 Max rebuild attempts reached for ${jid}`);
            return;
        }
        
        queueData.attempts++;
        console.log(`🔨 Rebuilding session for ${jid} (Attempt: ${queueData.attempts})`);
        
        // Method 1: Presence subscription
        await subscribeToPresence(jid);
        
        // Method 2: Send presence update
        await adams.sendPresenceUpdate("available", jid);
        
        // Method 3: If it's a group, build member sessions
        if (jid.includes('@g.us')) {
            await buildGroupMemberSessions(jid);
        }
        
        // Method 4: Try to fetch chat/group metadata
        try {
            if (jid.includes('@g.us')) {
                await adams.groupMetadata(jid);
            }
        } catch (metaError) {
            console.log(`ℹ️ Could not fetch metadata for ${jid}`);
        }
        
        // Mark session as rebuilt
        sessionTracking.set(jid, {
            ready: true,
            lastActivity: Date.now(),
            method: 'rebuilt',
        ...
        // Clear errors
        if (sessionErrors.has(jid)) {
            sessionErrors.get(jid).needsRebuild = false;
        }
        
        sessionBuildQueue.delete(jid);
        console.log(`✅ Session rebuilt successfully for ${jid}`);
        
    } catch (error) {
        console.error(`❌ Session rebuild failed for ${jid}:`, error);
        
        // Schedule retry if not max attempts
        const queueData = sessionBuildQueue.get(jid);
        if (queueData && queueData.attempts < SESSION_CONFIG.MAX_RETRIES) {
            setTimeout(() => {
                rebuildSessionForJid(jid);
            }, SESSION_CONFIG.RETRY_DELAY * queueData.attempts);
        } else {
            sessionBuildQueue.delete(jid);
        }
    }
...
function isSessionReady(jid) {
    const sessionData = sessionTracking.get(jid);
    const errorData = sessionErrors.get(jid);
    
    if (errorData && errorData.needsRebuild) {
        return false;
    }
...
const RATE_LIMITS = {
    GROUP_MESSAGE_DELAY: 2000,
    USER_MESSAGE_DELAY: 1000,
...
function checkRateLimit(jid, isGroup = false, isCommand = false) {
    ...
    if (!isSessionReady(jid)) {
        console.log(`⏳ Session not ready for ${jid}, scheduling rebuild`);
        scheduleSessionRebuild(jid);
        return true; // Rate limit until session is ready
    }
...
setInterval(() => {
    ...
    // Clean old session tracking
    for (const [key, data] of sessionTracking.entries()) {
        if (now - data.lastActivity > RATE_LIMITS.CLEANUP_INTERVAL * 2) {
            sessionTracking.delete(key);
        }
    }
    
    // Clean old member sessions
    for (const [key, data] of memberSessions.entries()) {
        if (now - data.timestamp > RATE_LIMITS.CLEANUP_INTERVAL * 3) {
            memberSessions.delete(key);
        }
    }
    
    console.log(`🧹 Sessions: ${sessionTracking.size}, Groups: ${groupSessionBuilt.size}, Members: ${memberSessions.size}`);
...
app.get("/health", (req, res) => {
    const status = {
        ...
        activeSessions: sessionTracking.size,
        groupSessions: groupSessionBuilt.size,
        memberSessions: memberSessions.size,
        sessionErrors: sessionErrors.size,
        buildQueue: sessionBuildQueue.size,
...
async function main() {
    ...
    try {
        ...
        // Enhanced heartbeat with presence updates
        heartbeatInterval = setInterval(async () => {
            try {
                if (adams && adams.user) {
                    ...
                    // Update presence for all tracked sessions
                    for (const [jid, sessionData] of sessionTracking.entries()) {
                        if (sessionData.ready) {
                            try {
                                await adams.sendPresenceUpdate("available", jid);
                            } catch (presenceError) {
                                // If presence update fails, schedule session rebuild
                                trackSessionError(jid, presenceError);
                            }
                        }
                    }
                }
                    ...
                    promises.push((async () => {
                        try {
                            ...
                            if (error.message.includes('No sessions') || error.message.includes('SessionError')) {
                                trackSessionError(key.remoteJid, error);
                            }
                        }
                    })());
                }
        ...
        // ENHANCED MESSAGE HANDLER WITH ALL SESSION BUILDING METHODS
        adams.ev.on("messages.upsert", async ({ messages }) => {
            try {
                ...
                // 🔥 IMPLEMENT ALL SESSION BUILDING METHODS
                
                // Method 1: Mark that we received a message (helps with session building)
                console.log(`📨 Received message from ${jid}, updating session tracking`);
                
                // Method 2: Apply message delay (Let session build)
                if (!sessionTracking.has(jid)) {
                    console.log(`⏳ First message from ${jid}, applying session delay`);
                    await delayedResponse(jid);
                }
                
                // Method 3: Subscribe to presence
                await subscribeToPresence(jid);
                
                // Method 4: Build group member sessions if it's a group
                if (isGroup && !groupSessionBuilt.has(jid)) {
                    console.log(`👥 Building group member sessions for ${jid}`);
                    // Do this in background to not block message processing
                    buildGroupMemberSessions(jid).catch(error => {
                        console.error(`Failed to build group sessions for ${jid}:`, error);
                    });
                }

                // Store message with timestamp
                if (!ibraah.chats[jid]) ibraah.chats[jid] = [];
                ibraah.chats[jid].push({
                    ...ms,
                    timestamp: Date.now()
                });

                // Keep only last 100 messages per chat
                if (ibraah.chats[jid].length > 100) {
                    ibraah.chats[jid].shift();
                }
                ...
                if (ms.message?.protocolMessage?.type === 0) {
                    ...
                    const deletedMsg = ibraah.chats[jid].find(m => m.key.id === deletedId);
                    ...
                    ibraah.chats[jid] = ibraah.chats[jid].filter(m => m.key.id !== deletedId);
                }
            } catch (error) {
        ...
        // ENHANCED COMMAND PROCESSING WITH SESSION AWARENESS
        adams.ev.on("messages.upsert", async ({ messages }) => {
            const ms = messages[0];
            ...
            async function processQueues() {
                ...
                try {
                    const highPriorityBatch = messageQueues.highPriority.splice(0, RATE_LIMITS_PROCESSING.highPriority);
                    const normalBatch = messageQueues.normal.splice(0, RATE_LIMITS_PROCESSING.normal);
                    const lowPriorityBatch = messageQueues.lowPriority.splice(0, RATE_LIMITS_PROCESSING.lowPriority);
            ...
            if (verifCom && com) {
                ...
                if (cmd) {
                    ...
                    // 🔥 APPLY SESSION CHECKING FOR COMMANDS
                    if (!isSessionReady(origineMessage)) {
                        console.log(`🚨 Session not ready for command in ${origineMessage}, rebuilding...`);
                        scheduleSessionRebuild(origineMessage);
                        
                        // Send a quick response to acknowledge the command
                        try {
                            await adams.sendMessage(origineMessage, {
                                text: "⏳ Initializing session, please try again in a moment...",
                            ...
                            console.error("Failed to send session building message:", error);
                        }
                        return;
                    }
                    ...
                    try {
                        const repondre = async (text, options = {}) => {
                            ...
                            // Apply delay before responding to ensure session stability
                            if (!sessionTracking.has(origineMessage)) {
                                await delayedResponse(origineMessage);
                            }
                            ...
                            try {
                                ...
                                if (err.message.includes('No sessions') || err.message.includes('SessionError')) {
                                    trackSessionError(origineMessage, err);
                                }
                            }
                        };
                        ...
                        if (error.message.includes('No sessions') || error.message.includes('SessionError')) {
                            trackSessionError(origineMessage, error);
                        }
        ...
        // Connection event handlers with session rebuilding
        adams.ev.on("connection.update", ({ connection }) => {
            if (connection === "open") {
...
└─────────────────┈ ⳹`;
        ...
        const retryDelay = connectionRetries < 7 ? 
            [2000, 5000, 10000, 15000, 30000, 60000, 120000][connectionRetries] : 120000;
        
        connectionRetries++;
        isReconnecting = false;
        
        if (connectionRetries <= MAX_RETRIES) {
            console.log(`⏳ Retrying connection in ${retryDelay}ms (Attempt: ${connectionRetries})`);
            setTimeout(() => {
                main();
            }, retryDelay);
        } else {
